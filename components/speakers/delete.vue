<template>
  <div>
    <v-btn icon depressed color="error" @click="addPermModel = true"
    ...
    <v-dialog v-model="addPermModel" width="300">
      <v-form v-model="valid" ref="form" lazy-validation>
        <v-card>
          <v-card-title>
            Delete Speaker<v-icon>mdi-warning</v-icon>
            <v-spacer />
          ...
          <v-card-text class="text-center">
            <v-alert outlined type="error" prominent border="left">
              You are about to delete <b>{{ speaker.name }} {{ speaker.surname }}</b>
            </v-alert>
          </v-card-text>
          <v-card-actions>
  ...
  methods: {
    async submit() {
      if (this.$refs.form.validate()) {
        ...
        try {
          await this.$axios
            .delete("api/admin/speakers/" + this.speaker.id)
            .then((res) => {
              this.loading = false;
              this.color = "success";
              this.snackbar = true;
